...
Then simply configure, compile, and install PLUMED normally. For instructions on how to install PLUMED, see \installplumed[here]. For those who are already using PLUMED, I recommend making a new installation for PLUMED+INDUS.
...
As an alternative to using PLUMED's \texttt{RESTRAINT} to set the form of the biasing potential, the INDUS code makes available its own biasing potential of the following general form:
...
Then, in the PLUMED input file, $\mathcal{U}_{\text{bias}} = \mathcal{U}_{\text{bias}}(\tilde{N}_v)$ is passed to \texttt{RESTRAINT} as its \texttt{ARG}. This is done to ensure that the biasing forces are correctly passed to the PLUMED core.
...
PLUMED's \texttt{driver} is invoked as follows:
