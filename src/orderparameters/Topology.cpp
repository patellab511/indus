...
void Topology::readGromacsTopology(const std::string& top_file)
{
	...
	while ( getline(ifs, line) ) {
		...
		if ( num_tokens >= 3 and tokens[0] == "[" and tokens[2] == "]" ) {
		...
		if ( record_name == "atomtypes" and num_tokens >= 7 ) {
			// Create a new entry
		...
		else if ( record_name == "moleculetype" and num_tokens >= 2 ) {
		...
		else if ( record_name == "atoms" and num_tokens >= 6 ) {
		...
		else if ( record_name == "molecules" and num_tokens >= 2 ) {
			// Add new entry
...
void Topology::readReferenceStateFromGroFile(const std::string& gro_file)
{
	...
	if ( not top_file_.empty() and num_atoms_ > 0 ) {
		...
		// Check atom names
		if ( static_cast<int>(atom_names_.size()) == num_atoms_ ) {
...
) const
{
	...
	if ( target_flags.isType or target_flags.isName ) {
		...
		if ( target_flags.is_atom ) {
			...
			else if ( target_flags.isType and static_cast<int>(atoms_.size()) != num_atoms_ ) {
				std::stringstream err_ss;
				err_ss << "Error in Topology::getTargets\n"
			...
			else if ( target_flags.isName and static_cast<int>(atom_names_.size()) != num_atoms_ ) {
				std::stringstream err_ss;
				err_ss << "Error in Topology::getTargets\n"
		...
		// Range of indices specified in PLUMED format
		for ( unsigned i=1; i<target_tokens.size(); ++i ) {
			parseTargetIndices(target_tokens[i], target_indices);
		}
	}
	else if ( target_flags.isFile ) {
		readIndicesFromFile(file_name, target_indices);
	}
...
void Topology::parseTargetIndices(
	const std::string& target_serial_ranges, std::vector<int>& target_indices) const
{
	...
	std::string range_string, token;

	bool done = false;
	while ( ! done ) {
		// Look for a comma, which indicates the end of the current range
		next_comma = target_serial_ranges.find_first_of(",", current_pos);
		if ( next_comma != std::string::npos ) {
			range_string = target_serial_ranges.substr(current_pos, next_comma - current_pos);
		}
		else {
			// Didn't find a comma: must be parsing the last range
			range_string = target_serial_ranges.substr(current_pos);
			next_comma  = target_serial_ranges.size();
		}
		...
		if ( current_pos >= target_serial_ranges.size() ) {
			done = true;
		}
	}
}
...
void Topology::readIndicesFromFile(
...
) const
{
	...
	while ( std::getline(ifs, line) ) {
		...
		while ( ss >> token ) {
			// Skip commments and GROMACS directive lines
			if ( token[0] == '#' || token[0] == ';' || token[0] == '[' ) {
				break;
			}
			// Store index = serial - 1
			serial = std::atoi( token.c_str() );
			indices.push_back(serial - 1);
		}
	}
}
