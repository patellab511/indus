...
#include "xdrfile/xdrfile_trr.h" /* read trr files */
...
class XdrFileTools
{
 public:
	...
	void readFrameFromXyzFile(
		...
		// Output
		RvecArray& coords,                      // Atom coordinates: (x,y,z) [nm]
		std::vector<std::string>& atom_types,   // Atom types (includes index in molecule)
		std::vector<int>& atom_serial_numbers,  // Atom serial no. (start at 1) read directly from file
		std::vector<Real3>& derivatives,        // Derivatives/forces (if present in file; else vector is empty)
		Rvec box_lengths                        // Simulation box lengths [nm]
	) const;
